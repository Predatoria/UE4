...
void UControlRigBlueprint::PropagatePoseFromBPToInstances()
{
	if (UClass* MyControlRigClass = GeneratedClass)
	{
		...
		{
						if (InstanceRig->Hierarchy.ControlHierarchy.GetIndex(InputControl.Name) != INDEX_NONE)
						{
						...
						{
							UE_LOG(LogTemp, Error, TEXT("Couldn't update control %s, it was not found."), *InputControl.Name.ToString());
						}
		}
		}
	}

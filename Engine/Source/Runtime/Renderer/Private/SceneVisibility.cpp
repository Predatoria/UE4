...
void FSceneRenderer::PreVisibilityFrameSetup(FRHICommandListImmediate& RHICmdList)
{
	...
	for(int32 ViewIndex = 0;ViewIndex < Views.Num();ViewIndex++)
	{
		...
		// AMD_CHANGES_BEGIN - FidelityFX Super Resolution
		if (ShouldFidelityFXFSROverrideMipBias(View))
		{
			// apply negative mip bias determined by the upscaling ratio to improve the final image quality
			View.MaterialTextureMipBias = GetFidelityFXFSRMipBias(View);
		}
		// AMD_CHANGES_END

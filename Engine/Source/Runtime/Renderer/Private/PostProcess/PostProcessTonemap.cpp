...
#include "PostProcess/PostProcessing.h"
// AMD_CHANGES_BEGIN - FidelityFX Super Resolution
...
FCommonDomain BuildCommonPermutationDomain(const FViewInfo& View, bool bGammaOnly, bool bSwitchVerticalAxis, bool bMetalMSAAHDRDecode)
{
	...
	// AMD_CHANGES_BEGIN - FidelityFX Super Resolution
	static TConsoleVariableData<int32>* CVarPostFSRGrainIntensity = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.FidelityFX.FSR.Post.FilmGrain"));
	const bool bSkipGrainIntensity = IsFidelityFXFSRPassEnabled(View) && CVarPostFSRGrainIntensity->GetValueOnRenderThread();
	PermutationVector.Set<FTonemapperGrainIntensityDim>(Settings.GrainIntensity > 0.0f && !bSkipGrainIntensity);
	// AMD_CHANGES_END
...
FDesktopDomain RemapPermutation(FDesktopDomain PermutationVector, ERHIFeatureLevel::Type FeatureLevel)
{
	...
	{
		// AMD_CHANGES_BEGIN - FidelityFX Super Resolution
		// As FSR handles GrainIntensity after upscaling/sharpening, we need to switch off the tonemapper variant.
		// This line below doesn't allow for a tonemapper variant GrainJitterDim=true and GrainIntensity=false,
		// so we comment it out and enable the variant to be compiled.
		#if 0
		...
		// AMD_CHANGES_BEGIN - FidelityFX Super Resolution
		// As FSR handles ColorFringe after upscaling/sharpening, we need to switch off the tonemapper variant.
		// This line below doesn't allow for a tonemapper variant GrainJitterDim=true and FTonemapperColorFringeDim=false,
		// so we comment it out and enable the variant to be compiled.
		#if 0
...
FScreenPassTexture AddTonemapPass(FRDGBuilder& GraphBuilder, const FViewInfo& View, const FTonemapInputs& Inputs)
{
	...
	const bool bFFX_FSREnabled = IsFidelityFXFSRPassEnabled(View);
	// AMD_CHANGES_END

	{
		...
		if (!CommonDomain.Get<TonemapperPermutation::FTonemapperGammaOnlyDim>())
		{
			...
			// AMD_CHANGES_BEGIN - FidelityFX Super Resolution
			const bool bSkipColorFringe = bFFX_FSREnabled && CVarPostFSRColorFringe->GetValueOnRenderThread();
			DesktopPermutationVector.Set<TonemapperPermutation::FTonemapperColorFringeDim>(PostProcessSettings.SceneFringeIntensity > 0.01f && !bSkipColorFringe);
			// AMD_CHANGES_END
		}
		...
		// AMD_CHANGES_BEGIN - FidelityFX Super Resolution
		// TonemapperPermutation::RemapPermutation() conditionally (GrainJitter=true || GrainIntensity=true) overrides the FTonemapperColorFringeDim,
		// and is called for both determining runtime shader selection as well as load-time shader variant compilation.
		// Hence, if FSR is enabled and cvars are set, disable GrainIntensity and ChromAb here.
		if (bFFX_FSREnabled)
		{
			static TConsoleVariableData<int32>* CVarPostFSRGrainIntensity2 = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.FidelityFX.FSR.Post.FilmGrain"));

			if (CVarPostFSRColorFringe->GetValueOnRenderThread())
			{
				DesktopPermutationVector.Set<TonemapperPermutation::FTonemapperColorFringeDim>(false);
			}

			if (CVarPostFSRGrainIntensity2->GetValueOnRenderThread())
			{
				TonemapperPermutation::FCommonDomain CommonDomain2 = DesktopPermutationVector.Get<TonemapperPermutation::FCommonDomain>();
				CommonDomain2.Set<TonemapperPermutation::FTonemapperGrainIntensityDim>(false);
				DesktopPermutationVector.Set<TonemapperPermutation::FCommonDomain>(CommonDomain2);
			}
		}
		// AMD_CHANGES_END
	}

...
TAutoConsoleVariable<int32> CVarUpscaleQuality(
	...
	TEXT(" 6: Contrast Adaptive Sharpening - CAS (AMD)\n"),
	// AMD_CHANGES_END
	ECVF_Scalability | ECVF_RenderThreadSafe);
...
public:
	...
	// AMD_CHANGES_BEGIN - FidelityFX CAS Integration
	class FNonLinearDimension : SHADER_PERMUTATION_BOOL("NONLINEAR");
	class FUpscaleDimension : SHADER_PERMUTATION_BOOL("UPSCALE");
	using FPermutationDomain = TShaderPermutationDomain<FMethodDimension, FNonLinearDimension, FUpscaleDimension>;
	// AMD_CHANGES_END
...
EUpscaleMethod GetUpscaleMethod()
{
	...
	// AMD_CHANGES_BEGIN - FidelityFX CAS Integration
	return static_cast<EUpscaleMethod>(FMath::Clamp(Value, 0, static_cast<int32>(EUpscaleMethod::CAS)));	
	// AMD_CHANGES_END
}
...
FScreenPassTexture AddUpscalePass(FRDGBuilder& GraphBuilder, const FViewInfo& View, const FUpscaleInputs& Inputs)
{
	...
	// AMD_CHANGES_BEGIN - FidelityFX CAS Integration
	PassParameters->UpscaleSoftness = FMath::Clamp(Inputs.CasSharpeningPass ? View.FinalPostProcessSettings.CasSharpening : CVarUpscaleSoftness.GetValueOnRenderThread(), 0.0f, 1.0f);
	// AMD_CHANGES_END
	...
	// AMD_CHANGES_BEGIN - FidelityFX CAS Integration
	const bool nonLinearColor = PassParameters->SceneColorTexture->Desc.Format != PF_FloatRGBA;
	const bool upscale = InputViewport.Rect != OutputViewport.Rect;
	PixelPermutationVector.Set<FUpscalePS::FNonLinearDimension>(nonLinearColor);
	PixelPermutationVector.Set<FUpscalePS::FUpscaleDimension>(upscale);
	// AMD_CHANGES_END

...
void AddPostProcessingPasses(FRDGBuilder& GraphBuilder, const FViewInfo& View, const FPostProcessingInputs& Inputs)
{
	...
	enum class EPass : uint32
	{
		...
		// AMD_CHANGES_BEGIN - FidelityFX CAS Integration
		CASSharpening,
		// AMD_CHANGES_END
		MAX
	};
	...
	const TCHAR* PassNames[] =
	{
		...
		// AMD_CHANGES_BEGIN - FidelityFX Super Resolution
		TEXT("FidelityFX-SuperResolution1"),
		// AMD_CHANGES_BEGIN - FidelityFX CAS Integration
		TEXT("CASSharpening")
		// AMD_CHANGES_END
	};
	...
	// AMD_CHANGES_BEGIN - FidelityFX Super Resolution
	const bool bFFX_FSR_Enabled = IsFidelityFXFSRPassEnabled(View);
	const bool bFFX_FSR_PrimaryUpscale = bFFX_FSR_Enabled && View.PrimaryScreenPercentageMethod == EPrimaryScreenPercentageMethod::FFX_FSR;
	const bool bFFX_FSR_SecondaryUpscale = bFFX_FSR_Enabled && !bFFX_FSR_PrimaryUpscale && View.Family->SecondaryScreenPercentageMethod == ESecondaryScreenPercentageMethod::FFX_FSR;
	// AMD_CHANGES_BEGIN - FidelityFX CAS Integration
	PassSequence.SetEnabled(EPass::CASSharpening, View.FinalPostProcessSettings.EnableCasSharpening && !(PassSequence.IsEnabled(EPass::PrimaryUpscale) && GetUpscaleMethod() == EUpscaleMethod::CAS));
	// FSR overrides the primary/secondary upscales if specified
	PassSequence.SetEnabled(EPass::PrimaryUpscale, !bFFX_FSR_Enabled && (PaniniConfig.IsEnabled() || (View.PrimaryScreenPercentageMethod == EPrimaryScreenPercentageMethod::SpatialUpscale && PrimaryViewRect.Size() != View.GetSecondaryViewRectSize())));
	PassSequence.SetEnabled(EPass::SecondaryUpscale, !bFFX_FSR_Enabled && View.RequiresSecondaryUpscale());
	
	PassSequence.SetEnabled(EPass::FidelityFXSuperResolution, bFFX_FSR_Enabled);
	// AMD_CHANGES_END
	...
	if (PassSequence.IsEnabled(EPass::PrimaryUpscale))
	{
		...
		// AMD_CHANGES_BEGIN - FidelityFX Super Resolution
		PassInputs.Stage = (PassSequence.IsEnabled(EPass::SecondaryUpscale) || bFFX_FSR_SecondaryUpscale)
		// AMD_CHANGES_END
			? EUpscaleStage::PrimaryToSecondary
			: EUpscaleStage::PrimaryToOutput;
	...
	// AMD_CHANGES_BEGIN - FidelityFX Super Resolution
	if (PassSequence.IsEnabled(EPass::FidelityFXSuperResolution))
	{
		FFSRInputs PassInputs;
		PassSequence.AcceptOverrideIfLastPass(EPass::FidelityFXSuperResolution, PassInputs.OverrideOutput);
		PassInputs.SceneColor = SceneColor;
		SceneColor = AddFidelityFXSuperResolutionPass(GraphBuilder, View, PassInputs);
	}
	// AMD_CHANGES_END
	...
	// AMD_CHANGES_BEGIN - FidelityFX CAS Integration
	if (PassSequence.IsEnabled(EPass::CASSharpening))
	{
		FUpscaleInputs PassInputs;
		PassSequence.AcceptOverrideIfLastPass(EPass::CASSharpening, PassInputs.OverrideOutput);

		PassInputs.SceneColor	     = SceneColor;
		PassInputs.Method 	     = EUpscaleMethod::CAS;
		PassInputs.Stage	     = EUpscaleStage::PrimaryToOutput;
		PassInputs.PaniniConfig	     = PaniniConfig;
		PassInputs.CasSharpeningPass = true;

...
float UNetConnection::GetTimeoutValue()
{
	...
	if (Driver->bNoTimeouts)
	{
		...
		return bPendingDestroy ? 2.0f : MAX_FLT;
	}
#endif
	...
	if ((State != USOCK_Pending) && (bPendingDestroy || (OwningActor && OwningActor->UseShortConnectTimeout())))
	{
		...
		Timeout = bPendingDestroy ? 2.0f : ConnectionTimeout;
	}
...
TAutoConsoleVariable<int32>CVarForceTimeoutTest(
	TEXT("net.forceTimeoutTest"),
	0,
	TEXT("Debug crap"),
	ECVF_SetByConsole
);
...
void UNetConnection::Tick(float DeltaSeconds)
{
	...
	if ((CurrentRealtimeSeconds - LastReceiveRealtime) > Timeout || CVarForceTimeoutTest->GetInt() == 1)
	{

...
static TAutoConsoleVariable<int32> CVarEnableFFXFSRPrimaryUpscale(
	TEXT("r.FidelityFX.FSR.PrimaryUpscale"),
	0,
	TEXT("Enable FidelityFX Super Resolution for Primary Upscale"),
	ECVF_RenderThreadSafe);
static TAutoConsoleVariable<int32> CVarEnableFFXFSRSecondaryUpscale(
	TEXT("r.FidelityFX.FSR.SecondaryUpscale"),
	0,
	TEXT("Enable FidelityFX Super Resolution for Secondary Upscale"),
	ECVF_RenderThreadSafe);
// AMD_CHANGES_END
...
#endif
{
	...
	// AMD_CHANGES_BEGIN - FidelityFX Super Resolution
	if (CVarEnableFFXFSRPrimaryUpscale.GetValueOnAnyThread() && GetFeatureLevel() >= ERHIFeatureLevel::SM5)
	{
		// Override TAA upscale if AMD-FSR is switched on.
		PrimaryScreenPercentageMethod = EPrimaryScreenPercentageMethod::FFX_FSR;
	}
	// AMD_CHANGES_END
...
// AMD_CHANGES_BEGIN - FidelityFX CAS Integration
		LERP_PP(CasSharpening);
		LERP_PP(EnableCasSharpening);
// AMD_CHANGES_END
	...
	TemporalUpscalerInterface(nullptr)
{
	...
	// AMD_CHANGES_BEGIN - FidelityFX Super Resolution
	if (CVarEnableFFXFSRSecondaryUpscale.GetValueOnAnyThread() && GetFeatureLevel() >= ERHIFeatureLevel::SM5)
	{
		SecondaryScreenPercentageMethod = ESecondaryScreenPercentageMethod::FFX_FSR;
	}
	// AMD_CHANGES_END
}

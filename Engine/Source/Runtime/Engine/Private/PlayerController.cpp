...
void APlayerController::SetNetSpeed(int32 NewSpeed)
{
	...
	{
		Player->CurrentNetSpeed = 1000000;// FMath::Clamp(NewSpeed, 1800, Driver->MaxClientRate);
...
void APlayerController::SetPlayer( UPlayer* InPlayer )
{
	...
	// cap outgoing rate to max set by serverd
	UNetDriver* Driver = GetWorld()->GetNetDriver();
	if( (ClientCap>=2600) && Driver && Driver->ServerConnection )
	{
		Player->CurrentNetSpeed = Driver->ServerConnection->CurrentNetSpeed = 1000000;// FMath::Clamp(ClientCap, 1800, Driver->MaxClientRate);
	}

...
public:
	...
	virtual uint64 GetCurrentFence() const final override 
	{ 
		bool bCheckVal = CurrentOrPendingFenceValue == CurrentFence || CurrentOrPendingFenceValue == CurrentFence + 1;
		if (!bCheckVal)
		{
			return CurrentOrPendingFenceValue; // added by Predatoria for Beasts of Bermuda.  Fixes frequent source of crashing if VSync is enabled and fps exceeds VSync rate.
		}
		check(CurrentOrPendingFenceValue == CurrentFence || CurrentOrPendingFenceValue == CurrentFence + 1); 
		return CurrentOrPendingFenceValue; 
	}
